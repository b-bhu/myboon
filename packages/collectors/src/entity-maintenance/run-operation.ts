...
export function parseOperatorCommand(args: readonly string[]): OperatorCommand {
  ...
  if (!kind || !id?.trim() || !actor?.trim() || args.length !== 3) {
    throw new Error('Usage: entity-catalog:operate <approve|reject|quarantine-alias|prepare-merge|rollback-alias> <finding-or-operation-id> <actor>')
  }
  if (kind === 'approve' || kind === 'reject' || kind === 'quarantine-alias' || kind === 'prepare-merge') {
    return { kind, findingId: id.trim(), actor: actor.trim() }
  }
  if (kind === 'rollback-alias') return { kind, operationId: id.trim(), actor: actor.trim() }
...
export async function executeOperatorCommand(db: SupabaseClient, command: OperatorCommand): Promise<Record<string, unknown>> {
  ...
  let rpc: { name: string, args: Record<string, string> }
