...
export const FinalDemoCaptions: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps, width } = useVideoConfig();
  const [captions, setCaptions] = useState<Caption[] | null>(null);
  const { delayRender, continueRender, cancelRender } = useDelayRender();
  const [handle] = useState(() => delayRender("Loading final demo captions"));

  const loadCaptions = useCallback(async () => {
    try {
      const response = await fetch(
        staticFile("final-submission/captions/demo.json"),
      );
  ...
  const productIsPrimary = frame >= 857 && frame < 4065;
  ...
  return (
    <AbsoluteFill
      style={{
        ...
        left: productIsPrimary ? 380 * scale : 0,
        paddingBottom: 68 * scale,
        pointerEvents: "none",
        right: productIsPrimary ? 430 * scale : 0,
        width: "auto",
        zIndex: 20,
      }}
    >
